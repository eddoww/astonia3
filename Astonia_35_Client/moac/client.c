...
#ifdef DB
int target_server=(192<<24)|(168<<16)|(42<<8)|(27<<0);
int base_server=(192<<24)|(168<<16)|(42<<8)|(27<<0);
#endif
#ifdef US
int target_server=(195<<24)|(90<<16)|(27<<8)|(38<<0);
int base_server=(195<<24)|(90<<16)|(27<<8)|(38<<0);
#endif
char username[40];
char password[16];
int zsinit;
	...
	if (len<1000) {
		...
		if (line[0]=='#') {
			if (!isdigit(line[1])) {
				...
				if (otext[MAXOTEXT-1].text) xfree(otext[MAXOTEXT-1].text);
...
#ifndef DEVELOPER
	if (!developer_server) target_server=base_server;
	else target_server=base_server;
#endif
	/*note("serverID=%d, would translate to: %u.%u.%u.%u",
	     ((*(unsigned int*)(buf+1))),
	     ((base_server+((*(unsigned int*)(buf+1)))>>24))&255,
	     ((base_server+((*(unsigned int*)(buf+1)))>>16))&255,
	     ((base_server+((*(unsigned int*)(buf+1)))>>8))&255,
	     ((base_server+((*(unsigned int*)(buf+1)))>>0))&255);*/
}
...
static void save_unique(void)
{
	...
	RegSetValueEx(hk,"gInput1",0,REG_DWORD,(void*)&unique,4);
	RegSetValueEx(hk,"gInput2",0,REG_DWORD,(void*)&usum,4);
}
...
static void load_unique(void)
{
	...
	RegQueryValueEx(hk,"gInput1",0,(void*)&type,(void*)&unique,(void*)&size);
	RegQueryValueEx(hk,"gInput2",0,(void*)&type,(void*)&usum,(void*)&size);
	...
	while (size>0 && panic++<20000) {
		//printf("pro: buf=%p, size=%d, *=%d\n",buf,size,buf[0]);
			...
			switch (buf[0]) {
				...
				case SV_TELEPORT:		sv_teleport(buf); len=9; break;

				case SV_MIRROR:                 sv_mirror(buf); len=5; break;
				case SV_PROF:			sv_prof(buf); len=P_MAX+1; break;
	...
	while (size>0 && panic++<20000) {
		//printf("pre: buf=%p, size=%d, *=%d [%d %d %d %d %d %d %d %d ]\n",buf,size,buf[0],buf[1],buf[2],buf[3],buf[4],buf[5],buf[6],buf[7],buf[8]);
			...
			switch (buf[0]) {
				...
				case SV_TELEPORT:		len=9; break;
				case SV_PROF:			len=P_MAX+1; break;
