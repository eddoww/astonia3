...
KEYTAB keytab[]= {
{ '1',0,0,1,0,"FIREBALL"	  ,TGT_CHR,CL_FIREBALL, 	V_FIRE },
...
{ '6',0,0,1,0,"BLESS SELF"        ,TGT_SLF,CL_BLESS,		V_BLESS },
{ '7',0,0,1,0,"HEAL"              ,TGT_CHR,CL_HEAL,		V_HEAL },
{ '8',0,0,1,0,"WARCRY"            ,TGT_SLF,CL_WARCRY,		V_WARCRY },
{ '9',0,0,1,0,"FIRERING"          ,TGT_SLF,CL_FIREBALL,		V_FIRE },
{ '0',0,0,1,0,"empty"             ,-1,-1,			-1 },

{ '1',0,0,1,1,"FIREBALL"          ,TGT_CHR,CL_FIREBALL,		V_FIRE },
...
{ '6',0,0,1,1,"BLESS SELF"        ,TGT_SLF,CL_BLESS,		V_BLESS },
{ '7',0,0,1,1,"HEAL"              ,TGT_CHR,CL_HEAL,		V_HEAL },
{ '8',0,0,1,1,"WARCRY"            ,TGT_SLF,CL_WARCRY,		V_WARCRY },
{ '9',0,0,1,1,"FIRERING"          ,TGT_SLF,CL_FIREBALL,		V_FIRE},
{ '0',0,0,1,0,"empty"             ,-1,-1,			-1 },

{ '1',0,0,0,1,"FIREBALL"          ,TGT_MAP,CL_FIREBALL,           V_FIRE },
...
{ '9',0,0,0,1,"FIRERING"          ,TGT_SLF,CL_FIREBALL,		  V_FIRE},
{ '0',0,0,1,0,"empty"             ,-1,-1,			   -1 },
};
...
struct special_tab special_tab[]={
	...
	{"Warcry",		0,0,CL_WARCRY,		TGT_SLF,	V_WARCRY},
        {"Fireball-CHAR",	0,1,CL_FIREBALL,	TGT_CHR,	V_FIRE},
	{"Fireball-MAP",	0,0,CL_FIREBALL,	TGT_MAP,	V_FIRE},
	{"Firering",		0,0,CL_FIREBALL,	TGT_SLF,	V_FIRE},
...
struct skill_over skill_over[20];
int tactics=0;

static void display_skill(void)
{
        int b,i,x,y,yt,bsx,bex,bsy,barsize,soc=0;
        char buf[256];

	bzero(skill_over,sizeof(skill_over));

	for (i=0; i<skltab_cnt; i++)
		if (skltab[i].v==V_TACTICS) tactics=skltab[i].curr;
		...
		switch(skltab[i].v) {
			...
			case V_OFFENSE:
			case V_DEFENSE:		sprintf(buf,"%d",skltab[i].curr+rage/4);
						break;

			case V_IMMUNITY:	sprintf(buf,"%2d/%2d",skltab[i].base,skltab[i].curr+rage/4);
						break;
			case V_WEAPON:
						...
						break;
			case V_HP:              if (lifeshield) sprintf(buf,"%d+%d/%2d/%2d",hp,lifeshield,skltab[i].base,skltab[i].curr);
		...
		if (tactics) {
			switch(skltab[i].v) {
				...
				case V_IMMUNITY:
				case V_RAGE:	
					skill_over[soc].ys=yt;
					skill_over[soc].ye=yt+10;
					sprintf(skill_over[soc].text,"%s: %d, Tactics bonus of %d (%d)",
						skltab[i].name,
						skltab[i].curr+
						(int)(tactics*0.15+0.01),
						(int)(tactics*0.15+0.01),
						tactics);
					soc++;
					break;
...
int get_teleport(int x,int y)
{
	...
	// mirror selector
	for (n=0; n<26; n++) {
		if (abs(mirror_pos[n*2]+TELE_X-x)<8 && abs(mirror_pos[n*2+1]+TELE_Y-y)<8) return n+201;
	}
...
void display_teleport(void)
{
	...
	if (!teleporter) return;
        ...
        dd_copysprite(53519,TELE_X+520/2,TELE_Y+320/2,14,0);
	if (clan_offset<16)      dd_copysprite(53520,TELE_X+102/2+340,TELE_Y+95/2+18,14,0);
	else if (clan_offset<32) dd_copysprite(53521,TELE_X+102/2+340,TELE_Y+95/2+18,14,0);
	else if (clan_offset<48) dd_copysprite(53522,TELE_X+102/2+340,TELE_Y+95/2+18,14,0);
	else                     dd_copysprite(53523,TELE_X+102/2+340,TELE_Y+95/2+18,14,0);
	...
	for (n=0; n<8; n++) {
		if (telsel==n+64) dx_copysprite_emerald(337+TELE_X,24+n*12+TELE_Y,3,2);
		else dx_copysprite_emerald(337+TELE_X,24+n*12+TELE_Y,3,1);
	}
	for (n=0; n<8; n++) {
		if (8+clan_offset+n>=60) continue;
                if (telsel==n+64+8) dx_copysprite_emerald(389+TELE_X,24+n*12+TELE_Y,3,2);
		else dx_copysprite_emerald(389+TELE_X,24+n*12+TELE_Y,3,1);
	}

	for (n=0; n<26; n++) {
		if (telsel==n+201) dx_copysprite_emerald(mirror_pos[n*2]+TELE_X,mirror_pos[n*2+1]+TELE_Y,1,2);
...
static void display_mouseover(void)
{
	...
	if (mousey>=496 && mousey<=551) {
		...
		if (mousex>=187 && mousex<=194) dd_drawtext(mousex,mousey-16,0xffff,DD_BIG|DD_FRAME|DD_CENTER,heal_text);
	...
	if (mousex>=115 && mousex<155) {
		for (n=0; n<20; n++) {
			if (!skill_over[n].ys) continue;
			if (mousey>=skill_over[n].ys && mousey<skill_over[n].ye) {
				dd_drawtext(mousex+16,mousey-4,0xffff,DD_BIG|DD_FRAME|DD_LEFT,skill_over[n].text);
			}
		}
	}
}
...
static void display_selfspells(void)
{
	...
	sprintf(heal_text,"Heal: Not active");
		...
		switch(ceffect[nr].generic.type) {
			...
			/*case 11:	step=50-50*(ceffect[nr].freeze.stop-tick)/(ceffect[nr].freeze.stop-ceffect[nr].freeze.start);
					...
					sprintf(freeze_text,"Freeze: %ds to go",(ceffect[nr].freeze.stop-tick)/24);
                                        break;*/
			case 10:	step=50*(tick-ceffect[nr].heal.start)/HEALDURATION;
					dd_push_clip();
					dd_more_clip(0,0,800,119+430);
					dd_copysprite(997,179+1*10,68+430+step,DDFX_NLIGHT,DD_NORMAL);
					dd_pop_clip();
					sprintf(heal_text,"Heal: %.1fs to go",(ceffect[nr].heal.start+HEALDURATION-tick)/24.0f);
                                        break;
...
static void display_rage(void)
{
	...
	step=50-50*rage/(value[0][V_RAGE]+(int)(value[0][V_TACTICS]*0.15+0.1));
	dd_push_clip();
	dd_more_clip(0,0,800,119+430);
	...
	sprintf(rage_text,"Rage: +%d",rage/4);
}
...
static void display(void)
{
	...
	extern int memptrused;
        ...
        if (sockstate<4 && ((t=time(NULL)-socktimeout)>10 || !originx)) {
                ...
                dd_copysprite(60,800/2,(600-120)/2,DDFX_NLIGHT,DD_CENTER);
                dd_copysprite(1,mousex,mousey+5,DDFX_BRIGHT,DD_CENTER);
                if (!kicked_out) {
			dd_drawtext_fmt(800/2,75,whitecolor,DD_LARGE|DD_CENTER|DD_FRAME,"Trying to establish connection. %d seconds...",t);
		}
                return;
        }
...
static void set_skltab(void)
{
	...
	for (flag=use=0,n=0; n<sizeof(itab)/sizeof(int); n++) {

		i=itab[n];

                if (flag && (i==0 || i==3 || i==7 || i==13 || i==18 || i==20 || i==27 || i==24 || i==40 || i==50)) {
			...
			skltab[use].button=0;
                ...
                else if (value[0][i] || value[1][i] || i==V_WEAPON || i==V_ARMOR || i==V_SPEED || i==V_LIGHT || i==V_OFFENSE || i==V_DEFENSE) {
			...
			if (i==V_WEAPON || i==V_ARMOR || i==V_SPEED || i==V_LIGHT || i==V_DEMON || i==V_COLD || i==V_OFFENSE || i==V_DEFENSE || i>=V_PROFBASE) {
...
static void exec_cmd(int cmd, int a)
{
	extern int display_help,display_quest;
		...
		case CMD_TELEPORT:	if (telsel==1042) {
						clan_offset+=16;
						if (clan_offset>48) clan_offset=0;
					} else {
						if (telsel>=64 && telsel<=200) cmd_teleport(telsel+clan_offset);
...
LRESULT FAR PASCAL _export main_wnd_proc(HWND wnd, UINT msg,WPARAM wparam, LPARAM lparam)
{
	...
	switch (msg) {
		...
		case WM_MOUSEWHEEL:
			...
			if (mousex<170 && mousey>435) {	// skill / depot / merchant
				while (delta>=120) { if (!con_cnt) set_skloff(0,skloff-1); else set_conoff(0,conoff-1); delta-=120; }
				while (delta<=-120) { if (!con_cnt) set_skloff(0,skloff+1); else set_conoff(0,conoff+1); delta+=120; }
				return 0;
			}

			if (mousex>220 && mousex<640 && mousey>435) {	// chat
				while (delta>=120) { dd_text_lineup(); dd_text_lineup(); dd_text_lineup(); delta-=120; }
				while (delta<=-120) { dd_text_linedown(); dd_text_linedown(); dd_text_linedown(); delta+=120; }
				return 0;
			}

			if (mousex>640 && mousey>435) {	// inventory
				while (delta>=120) { set_invoff(0,invoff-1); delta-=120; }
				while (delta<=-120) { set_invoff(0,invoff+1); delta+=120; }
				return 0;
			}
