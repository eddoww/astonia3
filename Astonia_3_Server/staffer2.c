/*
 * $Id: staffer2.c,v 1.4 2008/03/24 11:23:08 devel Exp $
 *
 * $Log: staffer2.c,v $
 * Revision 1.4  2008/03/24 11:23:08  devel
 * arkhata
 *
		...
		if (msg->type==NT_GIVE) {
			co=msg->dat1;
				...
				if (it[in].ID==IID_STAFF_HOLYRELIC && ppd && ppd->spiritbran_state<5) {
					...
					if (tmp==1 && !(ch[co].flags&CF_HARDCORE) && ch[co].saves<10) {
						ch[co].saves++;
		...
		if (msg->type==NT_CHAR) {
			...
			if (char_dist(cn,co)>10) { remove_message(cn,msg); continue; }
					...
					case 11:	quiet_say(cn,"If you ever have need of silver or gold for plating, I can offer reasonable trade rates.");
							ppd->broklin_state++; didsay=1;
                                                        break;
					case 12:	quiet_say(cn,"I can offer 1,000 gold units for 5,000 silver units.");
							ppd->broklin_state++; didsay=1;
                                                        break;
