/*
 * $Id: arkhata.c,v 1.3 2008/03/25 09:53:15 devel Exp $ (c) 2005 D.Brockhaus
 *
 * $Log: arkhata.c,v $
...
struct qa qa[]={
	...
	{{"raise","pulse",NULL},NULL,V_PULSE+100},
	{{"raise","armor",NULL},NULL,V_ARMORSKILL+100},
	{{"raise","armor","skill",NULL},NULL,V_ARMORSKILL+100},
	...
	{{"raise","body",NULL},NULL,V_BODYCONTROL+100},
	{{"raise","body","control",NULL},NULL,V_BODYCONTROL+100},
			...
			if (char_dist(cn,co)>10) { remove_message(cn,msg); continue; }
				...
				switch(ppd->rammy_state) {
					...
					case 15:	say(cn,"It is not their fault as they have been trained for that. They know only citizens of Arkhata, and enemies.");
							ppd->rammy_state++; didsay=1;
							break;
					case 16:	say(cn,"You must go see the captain in the fortress, hand him this letter. It will tell him who you are, and to find a solution to this issue");
		...
		if (msg->type==NT_TEXT) {
			...
			switch((didsay=analyse_text_driver(cn,msg->dat1,(char*)msg->dat2,co))) {
				case 2:		ppd=set_data(co,DRD_ARKHATA_PPD,sizeof(struct arkhata_ppd));
						...
						if (ppd && ppd->jaz_state>=6 && ppd->jaz_state<=7) { dat->last_talk=0; ppd->jaz_state=6; }
						break;
			}
			if (didsay) {
                                talkdir=offset2dx(ch[cn].x,ch[cn].y,ch[co].x,ch[co].y);
		...
		if (msg->type==NT_CHAR) {
			...
			if (char_dist(cn,co)>10) { remove_message(cn,msg); continue; }
				...
				switch(ppd->fiona_state) {
					...
					case 1:		say(cn,"Hello there stranger, and welcome to my Academy of the Fighting Arts. I can train your abillities here if you prove thine worth first.");
							questlog_open(co,67);
							ppd->fiona_state++; didsay=1;
							break;
					case 2:		say(cn,"I ventured into the vampire lair a few days ago, and suddenly my ring disappeared. And my students need me here so I can't retrieve it myself. Please bring it back to me.");
							ppd->fiona_state++; didsay=1;
							break;
					...
					case 5:		if (ch[co].level>80) {
							...
							didsay=1;
							break;
					case 6:		say(cn,"To fight my student say °c4enter°c0! (Offer valid only for level 80 and below)");
							ppd->fiona_state++;
							didsay=1;
							break;
		...
		if (msg->type==NT_CHAR) {
			...
			if (char_dist(cn,co)>10) { remove_message(cn,msg); continue; }
				...
				switch(ppd->ramin_state) {
					...
					case 1:		say(cn,"Hello Great Adventurer! Tidings of thy deed of returning Queen Fiona's ring have reached me. I believe you could help me too.");
							questlog_open(co,68);
					...
					case 4:		say(cn,"It seems it appeared out of nowhere and he is now afraid to even enter his own bedroom.");
							ppd->ramin_state++; didsay=1;
							break;
					case 5:		say(cn,"I would have sent a soldier for this but Rammy says he doesn't have any to spare from the fortress. May you go instead, explore this hole and destroy any dangers within it?");
							ppd->ramin_state++; didsay=1;
							break;
					...
					case 10:	if (ppd->rammy_state<14) say(cn,"Greetings my friend! I hear that you have helped the monks. Now Rammy has sent me news about trouble opening the fortress for a trade route, he is in need of thy help again. Please go and talk to him.");
							ppd->ramin_state++; didsay=1;
							break;
					case 11:	if (ch[co].level>=60 && ppd->rammy_state>=18) ppd->ramin_state++;
							else break;
					case 12:	say(cn,"Ah, it is good to see you again, %s!",ch[co].name);
							ppd->ramin_state++; didsay=1;
							break;
...
void arkhataskelly_dead(int cn,int co)
{
	...
	if (undead) {
		if ((undead%5)==0 || undead<10) log_char(co,LOG_SYSTEM,0,"%d down, %d to go. Beware of respawns!",80-undead,undead);
	} else {
		questlog_done(co,68);
		ppd->ramin_state=7;
			...
			if (char_dist(cn,co)>10) { remove_message(cn,msg); continue; }
				...
				switch(ppd->captain_state) {
					...
					case 2:		say(cn,"I believe we will need an entrance pass system to allow travellers to pass through the fortress safely. I can inform the guards of this.");
							ppd->captain_state++; didsay=1;
							break;
					case 3:		say(cn,"I need you to speak with the judge so that he can write the formal authorization letters for this. He will also tell you who needs a copy.");
							ppd->captain_state++; didsay=1;
							break;
					...
					case 7:		say(cn,"He was caught in the act, but even under torture we have failed to make him speak.");
							ppd->captain_state++; didsay=1;
							break;
					case 8:		say(cn,"The clerk has reported more notes missing, so he was not alone in this treachery. You should speak with the clerk.");
							ppd->captain_state++; didsay=1;
							break;
					case 9:		say(cn,"And a little warning. Your entrance pass is only valid in open areas of the fortress, some places my guard will attack you still.");
							ppd->captain_state++; didsay=1;
							break;
					case 10:	break;
					...
					case 3:		say(cn,"Here, take these three agreements, one for Ramin, one for Count Brannington and one for the fortress Captain.");
							...
							if (!(ppd->letter_bits&8) && !has_item(co,IID_ARKHATA_LETTER4)) {
								...
								if (in && !give_char_item(co,in)) destroy_item(in);
                                                        ...
                                                        ppd->judge_state++; didsay=1;
							break;
					case 4:		say(cn,"And for you, an entrance pass to the fortress, whilst carrying it, no guard will attack you.");
			...
			if (char_dist(cn,co)>10) { remove_message(cn,msg); continue; }
				...
				switch(ppd->jada_state) {
					...
					case 2:		say(cn,"I have discovered that the source of this evil that seems to penetrate our fortress is placed somewhere below it, in the cave system.");
                                                        ppd->jada_state++; didsay=1;
							break;
					case 3:		say(cn,"The hole in the corner there is our safe entrance to the cave system, we have only been able to search a small part of the caves. I ask you to go down there and find the source of this evil and bring it to me.");
                                                        ppd->jada_state++; didsay=1;
							break;
			...
			if (char_dist(cn,co)>10) { remove_message(cn,msg); continue; }
				...
				switch(ppd->pot_state) {
					...
					case 1:		say(cn,"Hello Stranger, I'm afraid someone stole a rather special pot I made on order from the Monk Thai Pan. I made it from iron blessed with holy water from a spring in the mountains outside the fortress.");
							...
							break;
					case 2:		say(cn,"It is quite a valuable pot, it can hold water in temperatures far below freezing without it turning to ice. Thai Pan has told me he could sense it's magic south of his temple, perhaps you should search the forest in that direction.");
                                                        ppd->pot_state++; didsay=1;
							break;
		...
		if (msg->type==NT_CHAR) {
			...
			if (char_dist(cn,co)>10) { remove_message(cn,msg); continue; }
				...
				switch(ppd->hunter_state) {
					case 0:		if (ppd->pot_state>0) ppd->hunter_state++;
							else break;
					case 1:		say(cn,"Hail adventurer! I see you are seeking for a ceremonial pot. Well that is an odd coincidence.");
					...
					case 5:		say(cn,"It's great to see you again adventurer! News of your deeds in Arkhata has reached even me.");
							questlog_open(co,77);
                                                        ppd->hunter_state++; didsay=1;
							break;
					...
					case 7:		say(cn,"Following hints from people who's seen it. Yet I have failed to seek out and slay the beast, it appears to move across a vast territory.");
                                                        ppd->hunter_state++; didsay=1;
							break;
					case 8:		say(cn,"I will reward you for slaying it, but its skin and the honor should be mine.");
                                                        ppd->hunter_state++; didsay=1;
							break;
		...
		if (msg->type==NT_CHAR) {
			...
			if (char_dist(cn,co)>10) { remove_message(cn,msg); continue; }
				...
				switch(ppd->thai_state) {
					...
					case 1:		say(cn,"Aaaaaaaaaaooooommmm... Oh, hello there friend. I'm Thai Pan, monk in this small place of worship. You are welcome to have a cup of green tea with me.");
							questlog_open(co,74);
					...
					case 2:		say(cn,"I know many stories and there is one in particular I think will interest thee.");
                                                        ppd->thai_state++; didsay=1;
							break;
					case 3:		say(cn,"It is said that the ruins on the shore right north-west of here was once a sanctuary for scholars of my order.");
                                                        ppd->thai_state++; didsay=1;
							break;
		...
		if (msg->type==NT_GIVE) {
			...
			if ((in=ch[cn].citem)) {	// we still have it
				ppd=set_data(co,DRD_ARKHATA_PPD,sizeof(struct arkhata_ppd));
					...
					if (ch[co].flags&CF_HARDCORE) w=ch[co].exp_used/500;
					else w=ch[co].exp_used/200;
					v=min(v,w);
					dlog(co,0,"got %d exp for thai pain quest 2",v);
					give_exp(co,v);
			...
			if (char_dist(cn,co)>10) { remove_message(cn,msg); continue; }
				...
				switch(ppd->clerk_state) {
					...
					case 2:		say(cn,"Three notes containing information about a transport from Brannington to Arkhata due tomorow night are missing. Most likely the traitors will pass these notes to the robbers tonight.");
                                                        ppd->clerk_state++; didsay=1;
							break;
					case 3:		say(cn,"Are you able to find these traitors in time? You will have three hours (Astonia time)! Say °c04Aye°c0 when you are ready!");
                                                        ppd->clerk_state++; didsay=1;
							break;
		...
		if (msg->type==NT_CHAR) {
			...
			if (char_dist(cn,co)>10) { remove_message(cn,msg); continue; }
				...
				switch(ppd->trainer_state) {
					...
					case 1:		say(cn,"Greetings, adventurer who returned the ring to my Queen Fiona. As one who may train in this academy now, your loyalty is expected in return.");
							questlog_open(co,75);
					...
					case 4:		say(cn,"Now I believe the gang leader is torturing him, or worse, to learn our fighting secrets.");
                                                        ppd->trainer_state++; didsay=1;
							break;
					case 5:		say(cn,"I need you to go get the student back, before our secrets are given away. Time is short, so go now!");
                                                        ppd->trainer_state++; didsay=1;
							break;
					...
					case 7:		say(cn,"Thank thee great fighter. Now that my student is safe I can sleep at night. You have proven your skills once again. The door to the academy will always be open to you.");
							ppd->trainer_state++; didsay=1;
							break;
					case 8:		break;
		...
		if (msg->type==NT_CHAR) {
			...
			if (char_dist(cn,co)>10) { remove_message(cn,msg); continue; }
				...
				switch(ppd->kid_state) {
					...
					case 1:		say(cn,"You must have been sent to rescue me! Oh how glad I am to see you! Please open this cage and let me out!");
		...
		if (msg->type==NT_CHAR) {
			...
			if (char_dist(cn,co)>10) { remove_message(cn,msg); continue; }
				...
				switch(ppd->krenach_state) {
					...
					case 1:		say(cn,"So you have met my grandson? And he is well? Oh by the pickaxe's tip you are one blessed human.");
							questlog_done(co,78);
					...
					case 2:		say(cn,"I am very happy to hear news of him. And to finally have someone to talk with.");
							ppd->krenach_state++; didsay=1;
							break;
					case 3:		say(cn,"10000g for a book is alot of money. I see you have taken good care of it. Here take these 5000g from me. All in all, the price of your adventure should be more reasonable now.");
