...
$Id: player_driver.c,v 1.2 2007/08/13 18:50:38 devel Exp $
...
Revision 1.4  2004/01/25 13:00:00  sam
fixed instant regeneration bug

Revision 1.3  2003/12/03 22:24:13  sam
fixed yet another typo

Revision 1.2  2003/10/13 14:12:34  sam
Added RCS tags
...
static int check_med_prio_task(int cn,int ac,int ac1,int ac2)
{
	switch(ac) {	
		...
		case PAC_PULSE:		if (do_pulse(cn)) return 1; else return error_state();
...
void player_driver(int cn,int ret,int last_action)
{
	...
	switch(player[nr]->action) {
		...
		case PAC_KILL:		if (!plr_attack_driver(cn,player[nr]->act1)) { player[nr]->action=0; do_idle(cn,4); } return;
