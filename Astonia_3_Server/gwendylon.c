/*
 *
 * $Id: gwendylon.c,v 1.8 2007/08/13 18:50:38 devel Exp $ (c) 2005 D.Brockhaus
 *
 * $Log: gwendylon.c,v $
 * Revision 1.8  2007/08/13 18:50:38  devel
 * fixed some warnings
 *
 * Revision 1.7  2007/05/26 13:20:31  devel
 * added carligar hooks to gwendylon
 *
...
struct qa qa[]={
	...
	{{"word",NULL},NULL,9},
	{{"oath",NULL},NULL,9},
	{{"raiseme",NULL},NULL,10},
	{{"hardcore",NULL},NULL,11},
        {{"i","accept","the","rules","and","wish","to","become","a","hardcore","character",NULL},NULL,12}
};
		...
		if (msg->type==NT_CHAR) {
			...
			if (char_dist(cn,co)>15) { remove_message(cn,msg); continue; }
				...
				if (has_empty_inventory(co) && !(ppd->flags&AF1_STORAGE_HINT)) {
					...
					case 0:		if ((ch[co].flags&CF_PAID) && ch[co].exp==0 && !(ch[co].flags&CF_HARDCORE)) quiet_say(cn,"°c3Hello, %s. Dost thou wish to become a °c4Hardcore°c3 character?",ch[co].name);
		...
		if (msg->type==NT_TEXT) {
			...
			switch((didsay=analyse_text_driver(cn,msg->dat1,(char*)msg->dat2,co))) {
				...
				case 11:        quiet_say(cn,"Hardcore is an option only available to paying players. A hardcore character does not earn saves when he levels, and he loses a lot more experience on death than a normal character. But he can train his skills higher than any other character.");
						...
						quiet_say(cn,"So, the rule is: It is thy choice to become a hardcore character, and thou must live with the consequences of that choice. The gods wilt not help thee. Dost thou accept these rules? [ °c4I accept the rules and wish to become a hardcore character°c0 ]");
                                                break;
				case 12:	if (!(ch[co].flags&CF_PAID)) {
						...
						if ((ch[co].flags&CF_HARDCORE)) {
							quiet_say(cn,"But thou art a hardcore character already.");
							break;
						}
						if (ch[co].exp) {
							quiet_say(cn,"But thou hast already earned experience. Thou mayest not become a hardcore character.");
							break;
						}
						quiet_say(cn,"So be it. Good luck, %s. Mayest thou never regret this decision.",ch[co].name);
						ch[co].flags|=CF_HARDCORE;
						ch[co].saves=0;
						dlog(co,0,"turned hardcore through James");
						break;
			}
			if (didsay) {
                                talkdir=offset2dx(ch[cn].x,ch[cn].y,ch[co].x,ch[co].y);
...
int james_raisehint(int cn,int doraise)
{
	...
	int offense=1,defense=1,immun=1,flash=1,fire=1,misc=1,warcry,pulse=1;
	...
	if (ch[cn].value[1][V_FLASH]>1 && ch[cn].value[1][V_FLASH]>ch[cn].value[1][V_FIREBALL]) { fire=0; flash=1; }
	if (ch[cn].value[1][V_FIREBALL]>1 && ch[cn].value[1][V_FIREBALL]>ch[cn].value[1][V_FLASH]) { fire=1; flash=0; }
	...
	if (offense) {
		if (can_raise(cn,V_ATTACK)) raise[V_ATTACK]+=1.0/(raise_cost(V_ATTACK,ch[cn].value[1][V_ATTACK],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR)));
		
		if (!ch[cn].value[1][V_ATTACK]) {
			if (can_raise(cn,V_BLESS)) raise[V_BLESS]+=1.0/(raise_cost(V_BLESS,ch[cn].value[1][V_BLESS],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*16);
			if (can_raise(cn,V_HEAL)) raise[V_HEAL]+=1.0/(raise_cost(V_HEAL,ch[cn].value[1][V_HEAL],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*16);
			if (can_raise(cn,V_FREEZE)) raise[V_FREEZE]+=1.0/(raise_cost(V_FREEZE,ch[cn].value[1][V_FREEZE],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*16);
			if (can_raise(cn,V_MAGICSHIELD)) raise[V_MAGICSHIELD]+=1.0/(raise_cost(V_MAGICSHIELD,ch[cn].value[1][V_MAGICSHIELD],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*16);
			if (can_raise(cn,V_FLASH)) raise[V_FLASH]+=1.0/(raise_cost(V_FLASH,ch[cn].value[1][V_FLASH],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*16);
			if (can_raise(cn,V_FIREBALL)) raise[V_FIREBALL]+=1.0/(raise_cost(V_FIREBALL,ch[cn].value[1][V_FIREBALL],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*16);
			//if (can_raise(cn,V_BALL)) raise[V_BALL]+=1.0/(raise_cost(V_BALL,ch[cn].value[1][V_BALL],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*16);
			if (can_raise(cn,V_PULSE)) raise[V_PULSE]+=1.0/(raise_cost(V_PULSE,ch[cn].value[1][V_PULSE],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*16);
		}
		
		v=get_fight_skill_skill(cn);
		if (can_raise(cn,v)) raise[v]+=1.0/(raise_cost(v,ch[cn].value[1][v],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*2);
	
		if (can_raise(cn,V_TACTICS)) raise[V_TACTICS]+=1.0/(raise_cost(V_TACTICS,ch[cn].value[1][V_TACTICS],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*4);
	
		if (!ch[cn].value[1][V_ATTACK]) {
			// skill
			if (can_raise(cn,skill[v].base1)) raise[skill[v].base1]+=1.0/(raise_cost(skill[v].base1,ch[cn].value[1][skill[v].base1],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*10/blessmod);
			if (can_raise(cn,skill[v].base2)) raise[skill[v].base2]+=1.0/(raise_cost(skill[v].base2,ch[cn].value[1][skill[v].base2],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*10/blessmod);
			if (can_raise(cn,skill[v].base3)) raise[skill[v].base3]+=1.0/(raise_cost(skill[v].base3,ch[cn].value[1][skill[v].base3],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*10/blessmod);

			// spells
			v=V_MAGICSHIELD;
			if (can_raise(cn,skill[v].base1)) raise[skill[v].base1]+=1.0/(raise_cost(skill[v].base1,ch[cn].value[1][skill[v].base1],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*10/2/blessmod);
			if (can_raise(cn,skill[v].base2)) raise[skill[v].base2]+=1.0/(raise_cost(skill[v].base2,ch[cn].value[1][skill[v].base2],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*10/2/blessmod);
			if (can_raise(cn,skill[v].base3)) raise[skill[v].base3]+=1.0/(raise_cost(skill[v].base3,ch[cn].value[1][skill[v].base3],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*10/2/blessmod);

			if (can_raise(cn,V_BLESS)) raise[V_BLESS]+=1.0/(raise_cost(V_BLESS,ch[cn].value[1][V_BLESS],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*40/3/3);
		} else {
			// skill
			if (can_raise(cn,skill[v].base1)) raise[skill[v].base1]+=1.0/(raise_cost(skill[v].base1,ch[cn].value[1][skill[v].base1],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*10/blessmod);
			if (can_raise(cn,skill[v].base2)) raise[skill[v].base2]+=1.0/(raise_cost(skill[v].base2,ch[cn].value[1][skill[v].base2],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*10/blessmod);
			...
			// tactics
			v=V_TACTICS;
			if (can_raise(cn,skill[v].base1)) raise[skill[v].base1]+=1.0/(raise_cost(skill[v].base1,ch[cn].value[1][skill[v].base1],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*20/blessmod);
			if (can_raise(cn,skill[v].base2)) raise[skill[v].base2]+=1.0/(raise_cost(skill[v].base2,ch[cn].value[1][skill[v].base2],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*20/blessmod);
			if (can_raise(cn,skill[v].base3)) raise[skill[v].base3]+=1.0/(raise_cost(skill[v].base3,ch[cn].value[1][skill[v].base3],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*20/blessmod);
		
			if (can_raise(cn,V_BLESS)) raise[V_BLESS]+=1.0/(raise_cost(V_BLESS,ch[cn].value[1][V_BLESS],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*40/3.5/3);
		}
	}

	if (defense) {
		if (can_raise(cn,V_PARRY)) raise[V_PARRY]+=1.0/(raise_cost(V_PARRY,ch[cn].value[1][V_PARRY],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR)));
		
		if (!ch[cn].value[1][V_PARRY]) {
			if (can_raise(cn,V_MAGICSHIELD)) raise[V_MAGICSHIELD]+=1.0/(raise_cost(V_MAGICSHIELD,ch[cn].value[1][V_MAGICSHIELD],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR)));
		}
		...
		if (!ch[cn].value[1][V_PARRY]) {
			...
			if (can_raise(cn,V_BLESS)) raise[V_BLESS]+=1.0/(raise_cost(V_BLESS,ch[cn].value[1][V_BLESS],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*40/2/3);
		} else {
			// skill
			if (can_raise(cn,skill[v].base1)) raise[skill[v].base1]+=1.0/(raise_cost(skill[v].base1,ch[cn].value[1][skill[v].base1],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*10/blessmod);
			if (can_raise(cn,skill[v].base2)) raise[skill[v].base2]+=1.0/(raise_cost(skill[v].base2,ch[cn].value[1][skill[v].base2],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*10/blessmod);
	...
	if (immun) {
		if (can_raise(cn,V_IMMUNITY)) raise[V_IMMUNITY]+=1.0/(raise_cost(V_IMMUNITY,ch[cn].value[1][V_IMMUNITY],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR)));
		if (can_raise(cn,V_TACTICS)) raise[V_TACTICS]+=1.0/(raise_cost(V_TACTICS,ch[cn].value[1][V_TACTICS],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*5);
		
		v=V_IMMUNITY;
		if (!ch[cn].value[1][V_TACTICS]) {
			// skill
			if (can_raise(cn,skill[v].base1)) raise[skill[v].base1]+=1.0/(raise_cost(skill[v].base1,ch[cn].value[1][skill[v].base1],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*5/blessmod);
			if (can_raise(cn,skill[v].base2)) raise[skill[v].base2]+=1.0/(raise_cost(skill[v].base2,ch[cn].value[1][skill[v].base2],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*5/blessmod);
			if (can_raise(cn,skill[v].base3)) raise[skill[v].base3]+=1.0/(raise_cost(skill[v].base3,ch[cn].value[1][skill[v].base3],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*5/blessmod);

			if (can_raise(cn,V_BLESS)) raise[V_BLESS]+=1.0/(raise_cost(V_BLESS,ch[cn].value[1][V_BLESS],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*20/3);
		} else {
			// skill
			if (can_raise(cn,skill[v].base1)) raise[skill[v].base1]+=1.0/(raise_cost(skill[v].base1,ch[cn].value[1][skill[v].base1],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*5/blessmod);
			if (can_raise(cn,skill[v].base2)) raise[skill[v].base2]+=1.0/(raise_cost(skill[v].base2,ch[cn].value[1][skill[v].base2],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*5/blessmod);
			...
			// tactics
			v=V_TACTICS;
			if (can_raise(cn,skill[v].base1)) raise[skill[v].base1]+=1.0/(raise_cost(skill[v].base1,ch[cn].value[1][skill[v].base1],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*25/blessmod);
			if (can_raise(cn,skill[v].base2)) raise[skill[v].base2]+=1.0/(raise_cost(skill[v].base2,ch[cn].value[1][skill[v].base2],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*25/blessmod);
			if (can_raise(cn,skill[v].base3)) raise[skill[v].base3]+=1.0/(raise_cost(skill[v].base3,ch[cn].value[1][skill[v].base3],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*25/blessmod);

			if (can_raise(cn,V_BLESS)) raise[V_BLESS]+=1.0/(raise_cost(V_BLESS,ch[cn].value[1][V_BLESS],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*20/1.2/3);
		}
	}

	if (flash && ch[cn].value[1][V_FLASH] && (ch[cn].flags&(CF_MAGE|CF_WARRIOR))!=(CF_MAGE|CF_WARRIOR)) {
		if (can_raise(cn,V_FLASH)) raise[V_FLASH]+=1.0/(raise_cost(V_FLASH,ch[cn].value[1][V_FLASH],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR)));
	...
	if (fire && ch[cn].value[1][V_FIREBALL] && (ch[cn].flags&(CF_MAGE|CF_WARRIOR))!=(CF_MAGE|CF_WARRIOR)) {
		if (can_raise(cn,V_FIREBALL)) raise[V_FIREBALL]+=1.0/(raise_cost(V_FIREBALL,ch[cn].value[1][V_FIREBALL],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR)));
	...
	if (pulse && ch[cn].value[1][V_PULSE] && (ch[cn].flags&(CF_MAGE|CF_WARRIOR))!=(CF_MAGE|CF_WARRIOR)) {
		if (can_raise(cn,V_PULSE)) raise[V_PULSE]+=1.0/(raise_cost(V_PULSE,ch[cn].value[1][V_PULSE],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR)));
	...
	if (warcry && ch[cn].value[1][V_WARCRY]) {
		if (can_raise(cn,V_WARCRY)) raise[V_WARCRY]+=1.0/(raise_cost(V_WARCRY,ch[cn].value[1][V_WARCRY],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*2);
	...
	if (misc) {
		for (v=0; v<V_MAX; v++) {
			...
			if (!raise[v] && can_raise(cn,v))
                                raise[v]=1.0/(raise_cost(v,ch[cn].value[1][v],(ch[cn].flags&(CF_MAGE|CF_WARRIOR))==(CF_MAGE|CF_WARRIOR))*weight);
		}
	}
	...
	for (v=0; v<V_MAX; v++) {
		if (raise[v] && raise[v]/mr<0.30 && !done[v] && v!=V_FREEZE && v!=V_FIREBALL && v!=V_FLASH && v!=V_PULSE) {	//v!=V_BALL &&
	...
	/*for (v=0; v<V_MAX; v++) {
		if (raise[v]) {
			log_char(cn,LOG_SYSTEM,0,"%s: %.2f",skill[v].name,raise[v]/mr);
		}
	}*/
...
void james_create_eq(int cn)
{
	...
	if (ch[cn].value[1][V_ARMORSKILL]) as=min(10,ch[cn].value[1][V_ARMORSKILL]/10+1);
	else as=0;
