...
{
	int cn,ret,vendor,mirror,area,ID,timeleft;
	...
	if (MAXITEM-used_items<512) {
        ...
        prof=prof_start(29); ret=find_login(name,password,&area,&cn,&mirror,&ID,vendor,&unique,htonl(player[nr]->addr),&timeleft);  prof_stop(29,prof);
	...
	if (ret==-2) { player_client_exit(nr,"You have been banned. Please email game@astonia.com for details."); return; }
        if (ret==-3) { player_client_exit(nr,"Character name or password wrong."); return; }
	...
	if (ret==-8) { player_client_exit(nr,"Please log onto your account management on www.astonia.com and update the account ownership information. Log in, click on 'Edit Address' and follow the instructions."); return; }
	if (ret==-9) { player_client_exit(nr,"Too many tries with bad passwords. Please come back later."); return; }
	if (ret==-10) { player_client_exit(nr,"The server is full. Please try again later (or pay for your account)."); return; }
	if (ret==-11) {
		int mn,hr,dy;
		char buf2[256];

		dy=timeleft/(60*60*24); timeleft-=dy*60*60*24;
		hr=timeleft/(60*60); timeleft-=hr*60*60;
		mn=timeleft/60; timeleft-=mn*60;

		sprintf(buf2,"You have been banned for %d days, %d hours, %d minutes, %d seconds.",dy,hr,mn,timeleft);
		player_client_exit(nr,buf2);
		return;
	}
	if (ret==-12) { player_client_exit(nr,"Your account is banned because you accumulated too much Karma."); return; }
