...
void enchant_item(int in,int cn)
{
	...
	if (it[in2].flags&IF_WNLHAND) {
        ...
        for (n=0; n<MAXMOD; n++) {
			...
			case V_LIGHT:	break;
			case V_COLD:	break;
			default:        if ((v=it[in2].mod_value[n])>0 && it[in2].mod_index[n]>=0 && v<20) {
					cost+=v*250+250;
					cnt++;
					}
		}
...
void special_potion(int in,int cn)
{
	...
	switch(type) {
		...
		case 12:	log_area(ch[cn].x,ch[cn].y,LOG_SYSTEM,0,16,"%s drinks a delicious apple juice.",ch[cn].name);
				remove_item_char(in);
				free_item(in);
                                return;

		case 13:        log_area(ch[cn].x,ch[cn].y,LOG_SYSTEM,0,16,"%s feels refreshed.",ch[cn].name);
				remove_item_char(in);
				free_item(in);
                                return;

		case 14:	log_area(ch[cn].x,ch[cn].y,LOG_SYSTEM,0,16,"%s cracks %s strong knuckles.",ch[cn].name,hisname(cn));
				remove_item_char(in);
				free_item(in);
                                return;
	...
	for (msg=ch[cn].msg; msg; msg=next) {
		...
		if (msg->type==NT_TEXT) {
			...
			if ((ptr=strstr(text,"trade with"))) {
				...
				dat->state=1;
				dat->c1ID=ch[co].ID; dat->s1ID=ch[co].sID;
                                dat->c2ID=ch[c2].ID; dat->s2ID=ch[c2].sID;
...
void sign(int in,int cn)
{
	...
	switch(nr) {
		case 0:		log_char(cn,LOG_SYSTEM,0,"Welcome to Long Tunnels");
...
int it_driver(int nr,int in,int cn)
{
	switch(nr) {
		...
		case IDR_EXPLORE:		explore_driver(in,cn); return 1;
		...
		case IDR_BETAITEMMAKER:		betaitemmaker(in,cn); return 1;
		case IDR_SIGN:			sign(in,cn); return 1;
