	...
	if (ppd->points>=ppd->base/4) {
		...
		if (in2 && it[in2].ID==IID_AREA25_TELEKEY) {
			switch(it[in2].drdata[0]) {
				...
				case 2:		if (ch[cn].saves<3) { ch[cn].saves++; log_char(cn,LOG_SYSTEM,0,"You received a save."); } break;
...
int it_driver(int nr,int in,int cn)
{
	switch(nr) {
		...
		case IDR_RWWKEY:	collect_keys(in,cn); return 1;
