...
static int cmd_complain(int cn,char *ptr)
{
	...
	sprintf(buf,"NW%d: %s",worldID,reason);
...
static int cmd_exterminate(int cn,char *ptr)		// 1=OK, 0=repeat
{
	...
	tell_chat(0,uID,1,"°c3You have been banned for %s.",reason);
        ...
        sprintf(buf,"NW%d: Punishment note from %s (%s), %s exterminated for %s",worldID,ch[cn].name,ch[cn].staff_code,realname,reason);
        write_scrollback(ch[cn].player,cn,buf,ch[cn].name,name);
...
static int cmd_punish(int cn,char *ptr)		// 1=OK, 0=repeat
{
	...
	tell_chat(0,uID,1,"°c3You have been punished for %s. You have lost karma, and you will get banned as a result of your punishment. The first few times the ban will be temporary, but eventually it will be permanent.",reason);
        ...
        sprintf(buf,"NW%d: Punishment note from %s (%s), %s punished for %s",worldID,ch[cn].name,ch[cn].staff_code,realname,reason);
        write_scrollback(ch[cn].player,cn,buf,ch[cn].name,name);
...
static int cmd_warn(int cn,char *ptr)		// 1=OK, 0=repeat
{
	...
	tell_chat(0,uID,1,"°c3You have been warned for %s. Further offenses will result in you getting karma and, eventually, banned.",reason);
        ...
        sprintf(buf,"NW%d: Warning note from %s (%s), %s warned for %s",worldID,ch[cn].name,ch[cn].staff_code,realname,reason);
        write_scrollback(ch[cn].player,cn,buf,ch[cn].name,name);
...
static int cmd_flag(int cn,char *ptr,unsigned long long flag)
{
	...
	if (!co) {
        ...
        switch(flag) {
		case CF_TUTOR:		fptr="tutor"; break;
		case CF_STAFF: 		fptr="staff"; break;
		case CF_GOD:		fptr="god"; break;
		case CF_TESTER:		fptr="tester"; break;
...
int command(int cn,char *ptr)	// 1=ok, 0=repeat
{
	...
	if (cmdcmp(ptr,"status",0)) {
		...
		if (ch[cn].flags&CF_PAID) {
			log_char(cn,LOG_SYSTEM,0,"Your Paid Account will expire in %d days.",(ch[cn].paid_till-time_now)/(60*60*24));
		} else log_char(cn,LOG_SYSTEM,0,"Basic Account");
