...
void build_shrine(int x,int y,int level)
{
	...
	switch(level) {
		...
		case 12:        build_shrine_sub(x,y,level,73,"Shrine of Welding",59394); break;
		...
		case 21:        break; //build_shrine_sub(x,y,level,41,"Shrine of Kindness",59396); break;
		...
		case 25:        build_shrine_sub(x,y,level,74,"Shrine of Welding",59394); break;
		...
		case 35:        build_shrine_sub(x,y,level,75,"Shrine of Welding",59394); break;
		...
		case 44:        build_shrine_sub(x,y,level,76,"Shrine of Welding",59394); break;
		...
		case 55:        break; //build_shrine_sub(x,y,level,42,"Shrine of Kindness",59396); break;
		case 56:        build_shrine_sub(x,y,level,77,"Shrine of Welding",59394); break;
		...
		case 65:        build_shrine_sub(x,y,level,78,"Shrine of Welding",59394); break;
		...
		case 77:        build_shrine_sub(x,y,level,79,"Shrine of Welding",59394); break;
		...
		case 89:        build_shrine_sub(x,y,level,80,"Shrine of Welding",59394); break;
		case 90:        break;
		...
		case 95:        build_shrine_sub(x,y,level,81,"Shrine of Welding",59394); break;
...
int can_give_mod(int in)
{
	...
	if (it[in].flags&IF_NOENHANCE) return 0;
		...
		switch(it[in].mod_index[n]) {
			case V_WEAPON:	continue;
			case V_ARMOR:	continue;
			case V_SPEED:	continue;
			case V_DEMON:	continue;
			case V_LIGHT:	continue;
			case V_OFFENSE:	continue;
			case V_DEFENSE:	continue;
			case V_COLD:	continue;
...
void shrine_welding(int in,int cn,int nr,int level,struct shrine_ppd *ppd)
{
	...
	n=m=0;
		...
		if (v==V_ARMOR) { n++; continue; }
		if (v==V_WEAPON) { n++; continue; }
		if (v==V_LIGHT) { n++; continue; }
		if (v==V_SPEED) { n++; continue; }
		if (v==V_DEFENSE) { n++; continue; }
		if (v==V_OFFENSE) { n++; continue; }
		if (v==V_COLD) { n++; continue; }
		if (v==V_DEMON) { n++; continue; }
		if (it[in1].mod_value[n]<0) { n++; continue; }

		v=it[in2].mod_index[m];
		if (v==V_ARMOR) { m++; continue; }
		if (v==V_WEAPON) { m++; continue; }
		if (v==V_LIGHT) { m++; continue; }
		if (v==V_SPEED) { m++; continue; }
		if (v==V_DEFENSE) { m++; continue; }
		if (v==V_OFFENSE) { m++; continue; }
		if (v==V_COLD) { m++; continue; }
		if (v==V_DEMON) { m++; continue; }
		if (it[in2].mod_value[m]<0) { m++; continue; }
...
void shrine_braveness(int in,int cn,int nr,int level,struct shrine_ppd *ppd)
{
	...
	give_exp_bonus(cn,cost);
	ch[cn].gold+=cost/10; stats_update(cn,0,cost/10);
	ch[cn].flags|=CF_ITEMS;
